...
#get the owner id
        ...
        if is_exist:
            ...
            return False,self.msg
        db=Database(instance_id=db_instance,name=db_name,business_id=db_bussiness,level=db_level,owner=owner_id,online_date=datetime.datetime.now())
        db.save()
        self.msg='Sucess'
        return True,self.msg
    def mod_database(self):
        db=Database.objects.get(id=self.db_id)
        db.instance_id=InstanceGet().get_instance_by_ip_port(self.db_ip, self.db_port)
